  ...
  return (
    <section className="border-b border-border bg-background px-4 py-20 sm:px-6 sm:py-24 lg:px-10 lg:py-32">
      <div className="mx-auto max-w-[1800px]">
        <p className="text-xs font-bold uppercase tracking-[0.28em] text-cyan sm:tracking-[0.38em]">Domaines</p>
        ...
        <ul className="mt-14">
          {creativeFields.map((field, index) => (
            <li key={field} className="group border-t border-border last:border-b">
              <div className="flex items-center gap-3 py-4 transition duration-300 group-hover:translate-x-2 sm:gap-4 sm:py-5">
                <span className="w-8 text-[0.65rem] font-bold uppercase tracking-[0.18em] text-gold opacity-0 transition group-hover:opacity-100 sm:w-10 sm:tracking-[0.25em]">{String(index + 1).padStart(2, "0")}</span>
                <span className="h-px w-0 bg-cyan transition-all duration-300 group-hover:w-8" aria-hidden="true" />
                <span className="font-display text-xl font-black uppercase leading-tight text-text-secondary transition group-hover:text-cyan sm:text-4xl sm:leading-none lg:text-5xl">{field}</span>
              </div>
            </li>
          ))}
        </ul>
